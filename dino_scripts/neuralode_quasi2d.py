...
from ode import ODE
'''
Changed the Conv3d and maxpool dim for everything -- double check
class NeuralODE(ODE):
...
class NeuralODE(ODE):
    ...
    def __init__(self, args, method='dopri5', adjoint=False, dtype=torch.float32):
        ...
        self.input_conv = nn.Sequential(
            nn.Conv3d(1, start_channels, kernel_size=(3,3,1), padding='same', padding_mode='circular'),
            nn.ReLU(),
            nn.Conv3d(start_channels, start_channels, kernel_size=(3,3,1), padding='same', padding_mode='circular'),
            nn.ReLU()
        )
        ...
        for i in range(self.num_layers):
            self.encoders.append(nn.Sequential(
                ...
                nn.Conv3d(channels[i], channels[i+1], kernel_size=(3,3,1), padding='same', padding_mode='circular'),
                nn.ReLU(),
                nn.Conv3d(channels[i+1], channels[i+1], kernel_size=(3,3,1), padding='same', padding_mode='circular'),
                nn.ReLU()
            ))
        ...
        for i in range(self.num_layers):
            ...
            self.decoders.append(nn.Sequential(
                nn.ConvTranspose3d(channels[self.num_layers-i], channels[self.num_layers-i-1], kernel_size=(2,2,1), stride=(2,2,1)),
                nn.Conv3d(channels[self.num_layers-i], channels[self.num_layers-i-1], kernel_size=(3,3,1), padding='same', padding_mode='circular'),
                #nn.Conv3d(2*channels[self.num_layers-i-1], channels[self.num_layers-i-1], kernel_size=(3,3,1), padding='same', padding_mode='circular'),
                nn.ReLU(),
                nn.Conv3d(channels[self.num_layers-i-1], channels[self.num_layers-i-1], kernel_size=(3,3,1), padding='same', padding_mode='circular'),
                nn.ReLU()
            ))
    ...
    def forward(self, t, u):
        ...
        u = u.view((-1, 1, self.Nx, self.Ny, self.Nz))
        ...
        for i, decoder in enumerate(self.decoders):
            ...
            #print(u.shape)
            #print (skip.shape)
            u = torch.cat([skip, u], dim=1) ## mad sus -> initially concatinated at dim 1 
